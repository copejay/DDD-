...
export class ChildWeaponBoxManager{
    ...
    constructor(WeaponBoxViewPrefab:Prefab,ParentNode:Node,ClickCallBack:(WeaponID:string)=>void){
    ...
    // //内部依赖：同步角色框列表
    // syncWeaponBoxList(WeaponInfoList){
    //     for(let i=0;i<this.WeaponBoxList.length;i++){
    //         let WeaponBox=this.WeaponBoxList[i];
    //         let WeaponInfo=WeaponInfoList[i];
    //         WeaponBox.syncName(WeaponInfo.name);
    //         WeaponBox.syncLevel(WeaponInfo.level);
    //     }
    // }
